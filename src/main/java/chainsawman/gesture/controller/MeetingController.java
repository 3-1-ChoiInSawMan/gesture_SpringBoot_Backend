...
public class MeetingController {
    ...
    @PostMapping("/start/calls/{callIdx}")
    ...
    @PostMapping("/{minutesIdx}/end")
    @Operation(summary = "회의록 종료 및 요약", description = "진행 중인 회의록을 종료하고 최종 데이터를 반환합니다.")
    public ResponseEntity<ApiResponse<MeetingEndResponse>> endMinutes(@PathVariable Long minutesIdx) {
        MeetingEndResponse result = meetingService.endMinutes(minutesIdx);
        return ResponseEntity.ok(ApiResponse.ok(result, "회의록이 종료되었습니다."));
    }

    @GetMapping("/rooms/{roomIdx}")
    @Operation(summary = "통화방 회의록 목록 조회", description = "해당 통화방의 전체 회의록 목록을 조회합니다.")
